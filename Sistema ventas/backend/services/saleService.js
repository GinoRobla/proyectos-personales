...
async function obtenerEstadisticas() {
    ...
    const inicioDelDia = new Date(ahora.getFullYear(), ahora.getMonth(), ahora.getDate()).toISOString();
    ...
    const ventasDeHoy = await Sale.count({ where: { createdAt: { [Op.between]: [inicioDelDia, finDelDia] } } });
    const ingresosDeHoy = (await Sale.sum('total', { where: { createdAt: { [Op.between]: [inicioDelDia, finDelDia] } } })) || 0;
